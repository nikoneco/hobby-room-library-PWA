...
function getMobileDockStatusText_(hasResults) {
  ...
  return `${count}冊 / ${getMobileDockViewLabel_(getCurrentViewMode_())}`;
}
...
function syncMobileAppDockState_(activeAction) {
  ...
  dock.setAttribute('aria-label', hasResults ? '主要操作と表示切替' : '主要操作');
  const status = document.getElementById('mobileDockStatus');
  if (status) {
    status.textContent = getMobileDockStatusText_(hasResults);
  }
  if (document.body) {
