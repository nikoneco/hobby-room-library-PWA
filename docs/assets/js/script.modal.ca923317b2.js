...
function buildPopupDetailLoadingHtml_(book) {
  ...
  if (book.detailError) {
    return `<div class="popup-detail-state error">${escapeHtml(book.detailError)}</div>`;
  }
  return `
    ...
    ? items.map((item, idx) => {
      ...
      return `
        <button type="button" class="series-list-item${isCurrent ? ' is-current' : ''}" data-series-index="${idx}" aria-label="${escapeHtml(item.title || 'シリーズ内の本')} の詳細を開く">
          ...
          ${buildSeriesListMetaHtml_(item, isCurrent)}
        </button>
      `;
    }).join('')
...
function buildSeriesListMetaHtml_(item, isCurrent) {
  const chips = [];
  const shelfLabel = typeof formatShelfLabel_ === 'function' ? formatShelfLabel_(item) : '';

  if (hasDisplayValue_(item && item.released)) {
    chips.push({ label: '発売', value: item.released, kind: 'released' });
  }
  if (shelfLabel) {
    chips.push({ label: '棚', value: shelfLabel, kind: 'location' });
  }
  if (isCurrent) {
    chips.push({ label: '', value: '表示中', kind: 'current' });
  }
  if (!chips.length) {
    chips.push({ label: '', value: '詳細未登録', kind: 'empty' });
  }

  return `
    <span class="series-list-meta">
      ${chips.map(chip => `
        <span class="series-list-meta-chip ${escapeHtml(chip.kind)}">
          ${chip.label ? `<span class="series-list-meta-label">${escapeHtml(chip.label)}</span>` : ''}
          <span class="series-list-meta-value">${escapeHtml(chip.value)}</span>
        </span>
      `).join('')}
    </span>
  `;
