...
function buildBookMetaPillClass_(kind) {
  return kind ? ` ${kind}` : '';
}

function buildBookMetaPillsHtml_(book, options) {
  const opt = options || {};
  const cardMode = opt.mode === 'card';
  const items = [];
  const shelfLabel = formatShelfLabel_(book);

  if (shelfLabel) items.push({ label: '棚', value: shelfLabel, kind: 'location' });
  if (hasDisplayValue_(book && book.released)) items.push({ label: '発売', value: book.released, kind: 'released' });
  if (!cardMode && hasDisplayValue_(book && book.price)) items.push({ label: '値', value: book.price, kind: 'price' });
  if (hasDisplayValue_(book && book.brand)) items.push({ label: 'レーベル', value: book.brand, kind: 'brand' });
  if (!cardMode && opt.includeIsbn && hasDisplayValue_(book && book.isbn)) items.push({ label: 'ISBN', value: book.isbn, kind: 'isbn' });

  if (!items.length) return '';

  return `
    <div class="book-meta-pills${opt.compact ? ' compact' : ''}${cardMode ? ' card-meta' : ''}">
      ${items.map(item => `
        <span class="book-meta-pill${buildBookMetaPillClass_(item.kind)}">
...
function buildPopupBookLeadHtml_(book) {
  ...
  if (hasDisplayValue_(book && book.author)) {
    ...
    parts.push(`
      <button
        type="button"
        class="popup-book-lead-search"
        data-detail-search-field="detailAuthor"
        data-detail-search-value="${escapeHtml(author)}"
        aria-label="作者「${escapeHtml(author)}」で検索"
      >${escapeHtml(author)}</button>
    `);
  }
  if (hasDisplayValue_(book && book.publisher)) {
    const publisher = String(book.publisher).trim();
    parts.push(`
      <button
        type="button"
        class="popup-book-lead-search"
        data-detail-search-field="detailPublisher"
        data-detail-search-value="${escapeHtml(publisher)}"
        aria-label="出版社「${escapeHtml(publisher)}」で検索"
      >${escapeHtml(publisher)}</button>
    `);
  }
  if (!parts.length) return '';

  return `<div class="popup-book-lead">${parts.join('<span aria-hidden="true"></span>')}</div>`;
}
...
function getLimitedGenreMeta_(meta, limit) {
...
function appendSummaryAccordion_(chipWrap, book) {
  ...
  const summaryId = `summaryText-${++summaryAccordionIdSeed_}`;
...
function updateViewToggleButtons_() {
  ...
  if (viewToggle) {
    viewToggle.dataset.currentMode = mode;
    viewToggle.setAttribute('aria-label', `表示モード。現在は${currentLabel}表示`);
  }
