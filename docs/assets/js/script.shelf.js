...
function renderShelfJumpNav_(groups, totalCount, immersive) {
  ...
  groups.forEach((group, index) => {
    const groupCount = group.count || getShelfGroupBookCount_(group);
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'shelf-jump-chip';
    button.dataset.shelfTarget = getShelfNavTargetId_(index);
    button.setAttribute('aria-label', `${group.label} ${groupCount}冊へ移動`);
    button.innerHTML = `
      <span class="shelf-jump-chip-label">${escapeHtml(group.label)}</span>
      <span class="shelf-jump-chip-count">${escapeHtml(String(groupCount))}冊</span>
    `;
...
function getShelfRoomMapStats_(groups) {
  const configs = getRoomMapShelfConfigs_();
  const stats = {
    shelves: configs.length,
    activeShelves: 0,
    mappedBooks: 0
  };
  ...
  configs.forEach(config => {
    ...
    if (count > 0) {
      stats.activeShelves += 1;
      stats.mappedBooks += count;
    }
  });
...
function formatShelfRoomMapMeta_(stats) {
  const safeStats = stats || { activeShelves: 0, shelves: 0, mappedBooks: 0 };
  return `${safeStats.activeShelves}/${safeStats.shelves}棚 ${safeStats.mappedBooks}冊`;
}
...
function createShelfRoomMapViewport_(groups, options) {
  ...
  getRoomMapShelfConfigs_().forEach(config => {
    ...
    button.classList.toggle('has-books', count > 0);
    button.disabled = targetIndex < 0;
    button.setAttribute(
      'aria-label',
      count ? `${config.label} 本棚 ${count}冊へ移動` : `${config.label} 本棚は登録なし`
    );
    button.innerHTML = `
...
function renderShelfRoomMap_(groups) {
  const stats = getShelfRoomMapStats_(groups);
  ...
  summaryMeta.className = 'shelf-room-map-summary-meta';
  summaryMeta.textContent = formatShelfRoomMapMeta_(stats);
...
function renderShelfRoomMapOverlay_(groups) {
  ...
  titleBlock.className = 'shelf-room-map-modal-title-block';
  ...
  meta.className = 'shelf-room-map-modal-meta';
  meta.textContent = formatShelfRoomMapMeta_(stats);
  ...
  hint.innerHTML = `
    <span>棚を選ぶと、その本棚へ移動します。</span>
    <span class="shelf-room-map-hint-meta">登録棚 ${escapeHtml(String(stats.activeShelves))}件</span>
  `;
...
function createShelfBookElement_(book, originalIndex, data, revealIndex) {
  ...
  function startShelfTap_(x, y) {
    tapStart = { x, y };
  }

  function clearShelfTap_() {
    tapStart = null;
  }

  function finishShelfTap_(event, x, y) {
    if (!tapStart) return false;
    const diffX = Math.abs(x - tapStart.x);
    const diffY = Math.abs(y - tapStart.y);
    tapStart = null;

    if (diffX > TAP_MOVE_LIMIT || diffY > TAP_MOVE_LIMIT) return false;

    suppressNextClick = true;
    window.setTimeout(function() {
      suppressNextClick = false;
    }, 420);
    openShelfBook_(event);
    return true;
  }
  ...
  button.addEventListener('pointerdown', function(e) {
    ...
    startShelfTap_(e.clientX, e.clientY);
  }, { passive: true });

  button.addEventListener('pointerup', function(e) {
    if (e.pointerType === 'mouse' || !tapStart) return;
    finishShelfTap_(e, e.clientX, e.clientY);
  });
  ...
  if (!window.PointerEvent) {
    button.addEventListener('touchstart', function(e) {
      ...
      if (!touch) return;
      startShelfTap_(touch.clientX, touch.clientY);
    }, { passive: true });

    button.addEventListener('touchend', function(e) {
      const touch = e.changedTouches && e.changedTouches[0];
      if (!touch) return;
      finishShelfTap_(e, touch.clientX, touch.clientY);
    });

    button.addEventListener('touchcancel', clearShelfTap_);
  }
...
function isGroupableSearchResultBook_(book) {
  if (!book || typeof book !== 'object') return false;
  const key = String(book.seriesKeyAuto || '').trim();
  if (!key) return false;
  if (book.isExtraSeries) return false;
  return !/^__extra__/i.test(key);
}

function getSearchResultSeriesTitle_(group) {
  if (!group || !Array.isArray(group.books) || !group.books.length) return 'シリーズ';
  const representativeBook = group.representativeBook || group.books[0] || {};
  return String(representativeBook.seriesSearchTitle || representativeBook.title || 'シリーズ').trim();
}

function getSearchResultSeriesGenreMeta_(group) {
  const books = group && Array.isArray(group.books) ? group.books : [];
  const seen = new Set();
  const genreMeta = [];

  books.forEach(book => {
    const items = book && Array.isArray(book.genreMeta) ? book.genreMeta : [];
    items.forEach(item => {
...
function buildSearchResultPresentation_(books) {
  const sourceBooks = Array.isArray(books) ? books : [];
  const entries = [];
  const groupByKey = new Map();

  sourceBooks.forEach((book, originalIndex) => {
    if (!isGroupableSearchResultBook_(book)) {
      entries.push({
        kind: 'book',
        book,
        originalIndex
    ...
    group.originalIndexes.push(originalIndex);
  });
  ...
  const presentationEntries = entries.map(entry => {
    ...
    if (entry.books.length < 2) {
      return {
        kind: 'book',
        book: entry.books[0],
        originalIndex: entry.originalIndexes[0]
      };
    }
  ...
  return {
    ...
    seriesGroupCount,
    rawBookCount: sourceBooks.length,
    displayItemCount: presentationEntries.length
  };
}

function createSearchResultSeriesCard_(group, viewMode, displayIndex) {
  const representativeBook = group && group.representativeBook ? group.representativeBook : {};
  const title = getSearchResultSeriesTitle_(group);
  const matchCount = Array.isArray(group && group.books) ? group.books.length : 0;
  const genreMeta = Array.isArray(group && group.genreMeta)
    ? group.genreMeta
    : getSearchResultSeriesGenreMeta_(group);
  ...
  const img = document.createElement('img');
  img.className = viewMode === 'list' ? 'list-thumb search-series-cover' : 'book-cover search-series-cover';
  img.alt = representativeBook.title || `${title}の代表表紙`;
  setupBookImageElement_(img, representativeBook, {
    track: true,
    trackKey: getBookImageTrackKey_(representativeBook, group.originalIndex),
    loading: displayIndex < (viewMode === 'list' ? 8 : 4) ? 'eager' : 'lazy',
    fetchPriority: displayIndex < (viewMode === 'list' ? 8 : 4) ? 'high' : 'low'
  });

  const content = document.createElement('div');
  content.className = viewMode === 'list' ? 'search-series-list-content' : 'card-info search-series-info';
  content.innerHTML = `
  ...
  if (viewMode === 'list') {
    ...
    row.className = 'list-row search-series-list-row';
    row.appendChild(img);
...
function syncSearchResultPresentationStats_(stats) {
  currentSearchResultPresentationStats = stats || null;
  if (!searchStatusState || searchStatusState.mode !== 'result') return;
  searchStatusState.seriesGroupCount = stats ? Number(stats.seriesGroupCount || 0) : 0;
  renderSearchStatus_();
...
function showResult(data) {
  ...
  if (!data || !Array.isArray(data) || data.length === 0) {
    syncSearchResultPresentationStats_(lastResultKind === 'search' ? {
      seriesGroupCount: 0,
      rawBookCount: 0,
      displayItemCount: 0
    } : null);
  ...
  const shouldGroupSearchResults = lastResultKind === 'search' && viewMode !== 'shelf';
  const presentation = shouldGroupSearchResults
    ? buildSearchResultPresentation_(data)
    : null;
  const renderItems = presentation ? presentation.entries : data;
  syncSearchResultPresentationStats_(presentation);
  const renderPerfToken = pwaPerfStart_('render:' + viewMode, {
    count: renderItems.length,
    rawCount: data.length,
    seriesGroups: presentation ? presentation.seriesGroupCount : 0
  });
  ...
  // カード/リストの既存集計だけを維持する。
  const trackedImageBooks = presentation
    ? renderItems.map(entry => entry.kind === 'series' ? entry.representativeBook : entry.book)
    : data;
  resetImageLoadStats_(viewMode === 'shelf' ? [] : trackedImageBooks);
  ...
  if (viewMode === 'shelf') {
    ...
    scheduleIncrementalResultRender_(result, renderItems, viewMode, function(start, end) {
      const fragment = document.createDocumentFragment();
      for (let idx = start; idx < end; idx += 1) {
      const entry = renderItems[idx];
      if (entry && entry.kind === 'series') {
        fragment.appendChild(createSearchResultSeriesCard_(entry, viewMode, idx));
        continue;
      }
      const book = presentation ? entry.book : entry;
      const originalIndex = presentation ? entry.originalIndex : idx;
      const card = document.createElement('div');
      card.className = 'book-card';
      setBookRevealIndex_(card, idx);
      setupBookOpenSurface_(card, book, originalIndex, data);
      ...
      setupBookImageElement_(img, book, {
        track: true,
        trackKey: getBookImageTrackKey_(book, originalIndex),
      ...
      info.innerHTML = `
        ...
        <div class="genre-chip-wrap card-chip-wrap">${buildGenreChips(book, { limit: 4 })}</div>
        ${buildBookMetaPillsHtml_(book, { mode: 'card' })}
        ${buildBookMemoHtml_(book)}
        <div class="book-open-cue" aria-hidden="true"><span>詳細</span>${uiIcon_('chevronDown', 'book-open-cue-icon')}</div>
      `;

      appendSummaryAccordion_(info.querySelector('.genre-chip-wrap'), book);
      setupBookKeyboardOpenSurface_(info.querySelector('.book-title'), book, originalIndex, data);
    ...
    scheduleIncrementalResultRender_(result, renderItems, viewMode, function(start, end) {
      ...
      setupBookOpenSurface_(row, book, originalIndex, data);
      ...
      titleBlock.innerHTML = `
        ...
        <div class="book-open-cue list-open-cue" aria-hidden="true"><span>詳細</span>${uiIcon_('chevronDown', 'book-open-cue-icon')}</div>
      `;

      appendSummaryAccordion_(titleBlock.querySelector('.genre-chip-wrap'), book);
      setupBookKeyboardOpenSurface_(titleBlock.querySelector('.book-title'), book, originalIndex, data);
      ...
      const detailId = `book-details-${shelfRenderRunId}-${idx}`;
      const btn = document.createElement('button');
      btn.className = 'accordion-toggle';
      btn.setAttribute('aria-expanded', 'false');
      btn.setAttribute('aria-controls', detailId);
      btn.setAttribute('aria-label', '補足情報を開く');
      btn.title = '補足情報を開く';
      btn.innerHTML = `<span class="accordion-label">補足</span>${uiIcon_('chevronDown', 'arrow')}`;
      row.appendChild(btn);
      ...
      btn.onclick = function(e) {
        ...
        btn.setAttribute('aria-label', expanded ? '補足情報を閉じる' : '補足情報を開く');
        btn.title = expanded ? '補足情報を閉じる' : '補足情報を開く';
