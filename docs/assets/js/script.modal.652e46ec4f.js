...
function buildPopupBookKickerHtml_(book, index, totalCount) {
  const items = [];
  const shelfLabel = typeof formatShelfLabel_ === 'function' ? formatShelfLabel_(book) : '';
  const position = Number.isFinite(index) && Number.isFinite(totalCount) && totalCount > 1
    ? `${index + 1}/${totalCount}`
    : '';

  if (shelfLabel) items.push(`棚 ${shelfLabel}`);
  if (position) items.push(`表示 ${position}`);
  if (hasDisplayValue_(book && book.released)) items.push(String(book.released).trim());
  if (hasDisplayValue_(book && book.brand)) items.push(String(book.brand).trim());

  if (!items.length) return '';

  return `
    <div class="popup-book-kicker" aria-label="本の位置と概要">
  ...
  info.innerHTML = `
    <div class="popup-book-detail">
      <div class="popup-book-head">
        ${buildPopupBookKickerHtml_(renderBook, popupIndex, popupData.length)}
